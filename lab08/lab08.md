...
Для запуска сервера `go run *.go`, для запуска клиента `go run *.go -mode client` из `stopnwait`, по умолчанию работают на портах 8080 (`-server-port`) и 8081 (`-client-port`) соответственно с задержкой в 5 секунд до повторной посылки (`-timeout s`). По умолчанию отправляется (`-send`) `client-resources/example.txt` 
...
Тесты в `checksum/checksum_test.go`, можно запустить через `go test -v .` из соответствующей директории

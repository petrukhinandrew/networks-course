...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - 0xdd160509
   - Так как DHCP использует широковещательную рассылку, сервера используют transaction id для определения, если их трафик был принят клиентом, а клиенты - чтобы игнорировать запросы, направленные не им (другим клиентам).
...
5. Каков IP-адрес вашего DHCP-сервера?
   - 100.114.32.2 (отправитель OFFER/ACK пакетов)
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   вашем эксперименте?
   - ![](images/lease.png)
   - Option 51, 1 час. Клиенты, которые отключились от сети не должны занимать IP адрес - его можно отдать новым клиентам
...
В `crc` `go run main.go -src <path>`. Пример по умолчанию в `sample.txt`

Вместо описания: логика написана в `processPacket` - пакет -> пакет + остаток -> "передача" (возможная инверсия бита) -> сравнение вычисления CRC с остатком
...
1. Независимо от номера кванта, $p(1-p)^3$ - А пытается передать пакет, а Б,В,Г - нет
2. Снова независимо от номера кванта, $3p(1-p)^3$ - один из трех квантов пытается передать пакет, остальные - не пытаются
3. Вероятность передачи в одном кванте $4p(1-p)^3$. Тогда вероятность не передать ничего в первые два кванта - $(1 - 4p(1-p)^3)^2$. При этом нужна передача в кванте 3, следовательно $(1 - 4p(1-p)^3)^2 \cdot 4p(1-p)^3$
4. Эффективность - это доля квантов, в которые произошла передача, значит $4p(1-p)^3$

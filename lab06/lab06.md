...
2. Определите длину (в байтах) для каждого поля UDP-заголовка, обращаясь к отображаемой
   ...
   - каждое поле занимает по 2 байта (4 16-ричных числа справа)
   - ![](images/12.png)
3. Значение в поле Length (Длина) – это длина чего?
   - Длина датаграммы - заголовка + данных
4. Какое максимальное количество байт может быть включено в полезную нагрузку UDP-пакета?
   - $2^{16} = 65535$ байт (8 байт заголовок и 65527 байт данных) в теории. Но учитывая, что датаграмма "оборачивается" в IP пакет, $65527 - 20 = 65507$ байт данных
...
7. Проверьте UDP-пакет и ответный UDP-пакет, отправляемый вашим хостом. Определите
   ...
   - порты "меняются местами", DNS запрос был отправленс порта 43892 на порт 53, а ответ был получен с порта 53 на порт 43892. 
...
Я устанавливал proftpd и использовал виртуальных пользователей как пользователей сервера
...
Запуск из `client` через `go run main.go`. Для подсказки можно вызвать `help` или `h` или `?`. Опять же, т.к. я пользуюсь локальными логином и паролем, пароль стерт в коде. 

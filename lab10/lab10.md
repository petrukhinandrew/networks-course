...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   - 20 байт в заголовке, значит 36 на полезную нагрузку
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Изменяются `Identification` и `Header Checksum`
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      -  Не меняются `Version, Header Length, Differentiated Services Field, Total Length, Flags, Fragment Offset, Protocol, Source Address, Destination Address`, которые и не должны меняться. 
      - Должны изменяться `Identification, Header Checksum` и раз в три запроса (по умолчанию для traceroute) `Time to live`
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - В одной серии ответов (3 пакетов TTL exceeded) id изменяются на 1, а TTL не изменяется. Для разных "серий" id могут сильно отличаться, как и TTL (252, 64, ...)
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - Ответ не фрагментируется, а запрос разбивается на 3 фрагмента

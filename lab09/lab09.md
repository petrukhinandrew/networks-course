...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Есть поле `unused` и исходный пакет, на который возвращается ошибка 
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Не происходит ошибок, поэтому не нужно возвращать исходный пакет (это поле, соответственно, отсутствует) 
   - ![](images/TR2.png)
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Таких каналов нет
   - Первый маршрутизатор локальной сети, последний ![](iplocation.net):
   - ![](images/loc.png)
